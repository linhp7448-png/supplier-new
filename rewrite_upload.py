...
if idx2 != -1:
    # Find the end of this block which ends right before: // Switch to Draft View
    # Actually the end of the block is:
    ...
    #         });
    #     });
    # Let's find "if (typeof imRender === 'function') {"
    end_str = "if (typeof imRender === 'function') {\n                  imRender();\n              }\n        });"
    end_idx = html.find(end_str, idx2)
    if end_idx != -1:
        end_idx += len(end_str)
    else:
        # try a different ending
        end_str2 = "imRender();\n              }\n        });"
        end_idx = html.find(end_str2, idx2)
        if end_idx != -1:
    ...
    print(f"Found block from {idx2} to {end_idx}")
...
'''
    ...
    html = html[:idx2] + new_block + html[end_idx:]
