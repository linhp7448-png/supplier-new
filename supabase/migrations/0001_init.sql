...
-- ============================================================
-- SCM Schema Integration (Hàng hóa & Báo giá)
-- ============================================================
...
-- 3. Table: raw_data (Báo giá liên kết Vendor x Item x Location)
-- vendor_code tham chiếu tới bảng vendor(code) của dự án Supplier hiện tại
create table if not exists raw_data (
    rid uuid default gen_random_uuid() primary key,
    loc_code text references locations(loc_code) on delete cascade,
    vendor_code text references vendor(code) on delete cascade,
    item_no text references items(item_no) on delete cascade,
...
declare t text;
begin
  foreach t in array array['locations','items','raw_data'] loop
    execute format('drop policy if exists p_all on %I', t);
    execute format('create policy p_all on %I for all to authenticated using (true) with check (true)', t);
  end loop;
end $$;

-- Thêm bucket item_images cho ảnh hàng hóa
insert into storage.buckets (id, name, public) values ('item_images','item_images', true)
on conflict (id) do nothing;

drop policy if exists sp_img_read on storage.objects;
create policy sp_img_read on storage.objects for select using (bucket_id='item_images');
drop policy if exists sp_img_write on storage.objects;
create policy sp_img_write on storage.objects for insert to authenticated with check (bucket_id='item_images');
drop policy if exists sp_img_del on storage.objects;
create policy sp_img_del on storage.objects for delete to authenticated using (bucket_id='item_images');
